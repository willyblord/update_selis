...
$(document).ready(function(){
	...
	if(isnum) {
		...
		location.href = "annual-group-bsc";
	}
	...
	$.ajax({
		url:"api/list-strategy-pillars-" + strat_id,
		method:"GET",
		dataType:"json",
		success:function(res) {
			var data = res;				
			$.each(data, function(i, option) {
				$('#pillar_id').append($('<option/>').attr("value", option.id).text(option.strategy_pillar));
			});
		}
	})
	...
	var dataTable = $('#table_data').DataTable({
		...
		'serverMethod': 'post',
		'ajax': {
			'url':'api/get-bsc-section-initiatives',
			'data': {
				stratId: param_id,
			}
		},
