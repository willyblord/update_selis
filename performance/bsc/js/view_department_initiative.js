...
$(document).ready(function(){
	...
	if(isnum) {
		...
		location.href = "annual-departmental-bsc";
	}
	...
	$.ajax({
		url:"api/list-department-initiatives-" + ctry_id + "-" + depart_id + "-" + strat_year,
		method:"GET",
		dataType:"json",
		success:function(res) {
			var data = res;				
			$.each(data, function(i, option) {
				$('#depart_initiative_id').append($('<option/>').attr("value", option.id).text(option.initiative));
			});
		}
	})
	...
	var dataTable = $('#table_data').DataTable({
		...
		'serverMethod': 'post',
		'ajax': {
			'url':'api/get-bsc-department-initiatives',
			'data': {
				stratId: param_id,
			}
		},
	...
	$(document).on('submit', '#initiative_form', function(event){
		...
		var depart_initiative_id = $('#depart_initiative_id').val();
		...
		if( (depart_initiative_id == '') || (target == '') || (value_impact == '') 
		...
		{			
			...
			const submitdata = {
				'depart_initiative_id': depart_initiative_id,
				'target': target,
	...
	$(document).on('click', '.update', function(){
		...
		$.ajax({
			...
			success:function(res) {
				if(res.success) {
					$('#addModal').modal('show');
					$('#depart_initiative_id').val(res.data.depart_initiative_id).trigger('change');
