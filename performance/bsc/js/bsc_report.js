...
$('#reset').click(function(){
    ...
    $("#threeyear_strategy").val('').trigger('change');
    $("#annual_year").val('').trigger('change');
...
$.ajax({
    url:"api/list-3yearstrategy-report",
    method:"GET",
    dataType:"json",
    success:function(res) {
        var data = res;	
        $('#threeyear_strategy option').remove();	
        $('#threeyear_strategy').append($('<option/>').attr("value", "").text("ALL"));	
        $.each(data, function(i, option) {
            $('#threeyear_strategy').append($('<option/>').attr("value", option.id).text(option.year_range));
        });
    }
})

$.ajax({
    url:"api/list-annualyearstrategy-report",
    method:"GET",
    dataType:"json",
    success:function(res) {
        var data = res;
        $('#annual_year option').remove();	
        $('#annual_year').append($('<option/>').attr("value", "").text("ALL"));		
        $.each(data, function(i, option) {
            $('#annual_year').append($('<option/>').attr("value", option.year).text(option.year));
        });
    }
})

$.ajax({
    url:"api/list-countriesstrategy-report",
    method:"GET",
    dataType:"json",
    success:function(res) {
        var data = res.data;	
...
$.ajax({
    url:"api/list-departmentstrategy-report",
    method:"GET",
    dataType:"json",
    success:function(res) {
        var data = res.data;
...
$(document).on('submit', '#filter_form', function(event){
    event.preventDefault();
        ...
        let threeyear_strategy = $('#threeyear_strategy').val();
        ...
        $("<div class='row'>"+
            "<div class='col-sm-6 col-md-3'>" +
                ...
                "<form method='post' id='myform' action='extract-pdf-strategy-initiatives-reports'>" +
                    "<input type='hidden' name='country' value='" + country + "' />" +
                    "<input type='hidden' name='department' value='" + department + "' />" +
                    "<input type='hidden' name='threeyear_strategy' value='" + threeyear_strategy + "' />" +
                    "<input type='hidden' name='annual_year' value='" + annual_year + "' />" +
            ...
            "<div class='col-sm-6 col-md-3'>" +
                "<div class='form-holder'>" +
                "<form method='post' id='myform_2' action='extract-excel-strategy-initiatives-reports'>" +
                    "<input type='hidden' name='country' value='" + country + "' />" +
                    "<input type='hidden' name='department' value='" + department + "' />" +
                    "<input type='hidden' name='threeyear_strategy' value='" + threeyear_strategy + "' />" +
                    "<input type='hidden' name='annual_year' value='" + annual_year + "' />" +
        ...
        var dataTable = $('#data_table').DataTable({
            ...
            'destroy': true,
            'ajax': {
                'url':'api/get-strategy_initiatives-report',
                'data': {
                    country: country,
                    department: department,
                    threeyear_strategy: threeyear_strategy,
            ...
            'createdRow': function( row, data, dataIndex ) {
                if ( data.figure ) { $(row).find('td:eq(6)').css({ "font-weight": "bold", "color": "#390505" }); }
                if ( data.weight ) { $(row).find('td:eq(7)').css({ "font-weight": "bold", "color": "#b01c2e" }); }
                if ( data.raw_score ) { $(row).find('td:eq(8)').css({ "font-weight": "bold", "color": "#005ca4" }); }
                if ( data.target_score ) { $(row).find('td:eq(9)').css({ "font-weight": "bold", "color": "#009114" }); }
                if ( data.computed_score ) { $(row).find('td:eq(10)').css({ "font-weight": "bold", "color": "#8b0063" }); }
            }
        });
});	
