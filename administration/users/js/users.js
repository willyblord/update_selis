...
$(document).ready(function(){
	...
	populateDropdown('country', 'api/list-countries', 'country_name');

	// Event listeners for dropdown changes
	$('#country').change(function() {
		var countryId = $(this).val();
		if (countryId !== '') {
			populateDropdown('division', 'api/list-divisions-' + countryId, 'division_name');
		} else {
			$('#division').empty().append($('<option>').attr('value', '').text('...'));
			$('#department').empty().append($('<option>').attr('value', '').text('...'));
			$('#unit').empty().append($('<option>').attr('value', '').text('...'));
			$('#section').empty().append($('<option>').attr('value', '').text('...'));
		}
	});
	...
	$('#division').change(function() {
		...
		if (divisionId !== '') {
			populateDropdown('department', 'api/list-departments-' + divisionId, 'department_name');
		} else {
			$('#department').empty().append($('<option>').attr('value', '').text('...'));
			$('#unit').empty().append($('<option>').attr('value', '').text('...'));
			$('#section').empty().append($('<option>').attr('value', '').text('...'));
	...
	$('#department').change(function() {
		...
		if (departmentId !== '') {
			populateDropdown('unit', 'api/list-units-' + departmentId, 'unit_name');
		} else {
			$('#unit').empty().append($('<option>').attr('value', '').text('...'));
			$('#section').empty().append($('<option>').attr('value', '').text('...'));
		}
	...
	$('#unit').change(function() {
		var unitId = $(this).val();
		if (unitId !== '') {
			populateDropdown('section', 'api/list-sections-' + unitId, 'section_name');
		} else {
			$('#section').empty().append($('<option>').attr('value', '').text('...'));
		}
	});
	...
	function load_roles() {
		...
		$.ajax({
			...
			success:function(res) {
				var data = res;				
				$.each(data, function(i, option) {
					$('#roles').append($('<option/>').attr("value", option.id).text(option.role_name));
				});
			}
		})
	}
	...
	$(document).on('submit', '#user_form', function(event){
		...
		if( (name == '') ||  (surname == '') || (country == '') || (division == '') || (department == '') || (email == '') )
		{
		...
		$.ajax({
			...
			success:function(res) {
				if(res.success) {
					...
					$('#country').val(res.data.country_id).trigger('change');
					$('#division').val(res.data.division_id).trigger('change');
					$('#department').val(res.data.department_id).trigger('change');
					$('#section').val(res.data.section_id).trigger('change');
					$('#unit').val(res.data.unit_id).trigger('change');
					...
					populateDropdown('division', 'api/list-divisions-' + res.data.country_id, 'division_name', res.data.division_id);
					populateDropdown('department', 'api/list-departments-' + res.data.division_id, 'department_name', res.data.department_id);
					populateDropdown('unit', 'api/list-units-' + res.data.department_id, 'unit_name', res.data.unit_id);
					populateDropdown('section', 'api/list-sections-' + res.data.unit_id, 'section_name', res.data.section_id);
