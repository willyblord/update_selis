...
$(document).ready(function(){
	...
	$('#reset').click(function(){
		...
		$("#budget_category").val('').trigger('change');
		$("#cashCategory").val('').trigger('change');
		$("#statusCheck").val('').trigger('change');
		$("#status").val('').trigger('change');
		$("#cashReqBy").val('').trigger('change');
		$("#cashDisbBy").val('').trigger('change');
	...
	$.ajax({
		url:"api/list-cash-report-budget",
		method:"GET",
		dataType:"json",
		success:function(res) {
			var data = res;
			$('#budget_category option').remove();	
				$('#budget_category').append($('<option/>').attr("value", "").text("ALL"));	
			$.each(data, function(i, option) {
	...
	$.ajax({
		url:"api/list-cash-report-category",
		method:"GET",
		dataType:"json",
		success:function(res) {
			var data = res;
			$('#cashCategory option').remove();	
				$('#cashCategory').append($('<option/>').attr("value", "").text("ALL"));	
			$.each(data, function(i, option) {
				$('#cashCategory').append($('<option/>').attr("value", option.category).text(option.category));
			});
		}
	})

	$.ajax({
		url:"api/list-cash-report-status",
		method:"GET",
		dataType:"json",
		success:function(res) {
			var data = res;
	...
	$.ajax({
		url:"api/list-cash-report-requestby",
		method:"GET",
		dataType:"json",
		success:function(res) {
			var data = res;
			$('#cashReqBy option').remove();	
				$('#cashReqBy').append($('<option/>').attr("value", "").text("ALL"));	
			$.each(data, function(i, option) {
				$('#cashReqBy').append($('<option/>').attr("value", option.id).text(option.requestBy));
			});
		}
	})
	...
	$(document).on('submit', '#filter_form', function(event){
		event.preventDefault();
			...
			let budget_category =  $('#budget_category').val();
			let cashCategory =  $('#cashCategory').val();
			let statusCheck =  $('#statusCheck').val();
			let status =  $('#status').val();
			let cashReqBy =  $('#cashReqBy').val();
			let cashDisbBy =  $('#cashDisbBy').val();
			...
			$("<div class='row'>"+
				"<div class='col-sm-6 col-md-3'>" +
					...
					"<form method='post' id='myform' action='extract-pdf-pettycash-reports'>" +
						"<input type='hidden' name='country' value='" + country + "' />" +
						"<input type='hidden' name='department' value='" + department + "' />" +
						"<input type='hidden' name='budget_category' value='" + budget_category + "' />" +
						"<input type='hidden' name='cashCategory' value='" + cashCategory + "' />" +
						"<input type='hidden' name='statusCheck' value='" + statusCheck + "' />" +
						"<input type='hidden' name='status' value='" + status + "' />" +
						"<input type='hidden' name='cashReqBy' value='" + cashReqBy + "' />" +
						"<input type='hidden' name='cashDisbBy' value='" + cashDisbBy + "' />" +
				...
				"<div class='col-sm-6 col-md-3'>" +
					"<div class='form-holder'>" +
					"<form method='post' id='myform_2' action='extract-excel-pettycash-reports'>" +
						"<input type='hidden' name='country' value='" + country + "' />" +
						"<input type='hidden' name='department' value='" + department + "' />" +
						"<input type='hidden' name='budget_category' value='" + budget_category + "' />" +
						"<input type='hidden' name='cashCategory' value='" + cashCategory + "' />" +
