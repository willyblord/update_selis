...
$(document).ready(function(){
	...
	$('#budget_topup_form .select2').select2({
        dropdownParent: $('.modals')
    });
	
	$('#hod_bal_button').click(function(){
		$('#budget_topup_form')[0].reset();
		$('.modal-title').text("Budget Request");
		$("#budget_to_deduct").val('').trigger('change');
		$("#budget_to_increase").val('').trigger('change');
		$('#operation').val("hod_request_budget");
	});
	...
	$.ajax({
		...
		success:function(res) {
			...
			$.each(data, function(i, option) {
				$('#budget_to_deduct').append($('<option/>').attr("value", option.id).text(option.budget_category));
				$('#budget_to_increase').append($('<option/>').attr("value", option.id).text(option.budget_category));
			});
		}
	})	
	...
	var dataTable = $('#request_data').DataTable({
		...
		'serverMethod': 'post',
		'ajax': {
			'url':'api/get-hod-pettycash-requests'
		},
		'columns': [   
			{ data: 'refNo'},
			{ data: 'category' },
	...
	$(document).on('submit', '#budget_topup_form', function(event){
		event.preventDefault();

		var budget_to_deduct = $('#budget_to_deduct').val();
		var budget_to_increase = $('#budget_to_increase').val();
		var amount = $('#amount').val();
		var description = $('#description').val();
		var id = $('#id2').val();
		
		if( (budget_to_deduct == '') || (budget_to_increase == '') ||  (amount == '') || (description == '') )
		{
	...
	var cacheData;
	var data = $('#autoDepartPercent').html();
	var auto_refresh = setInterval(
	function ()
	{
		$.ajax({
			url: 'api/get-hod-budgetspent',
			type: 'GET',
			data: data,
			dataType:"json",
			success: function(data) {
				if (data !== cacheData){
					cacheData = data;
					$('#autoDepartPercent').html(data.spent);
				}         
			}
		})
	}, 1000); 
